<script>
  ...
  let { visible = $bindable(false), sessionId = '', onClose = () => {} } = $props();
        ...
        {:else}
          {#each filteredEntries() as { key, value }}
            <div class="var-row">
              ...
              <button class="btn-edit" onclick={() => promptEdit(key, value)}>编辑</button>
              <button class="btn-delete" onclick={() => deleteKey(key)}>删除</button>
            </div>
            <div class="var-code">
