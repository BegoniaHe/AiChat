<script>
    ...
    async function applyBoundConfigIfAny() {
        ...
        try {
            ...
            if (window.appBridge) {
                ...
                window.appBridge.client = canInitClient(cfg)
                    ? new LLMClient(cfg)
                    : null;
            }
    ...
    function collectEditorData(base) {
        ...
        if (activeType === "openai" && openaiParamsEditor?.collectData) {
            ...
            data.boundProfileId =
                window.appBridge?.config?.getActiveProfileId?.() ||
                current.boundProfileId ||
                null;
            return { ...current, ...data };
        }
    ...
    async function handleImportFile(e) {
        ...
        const fileBaseName = String(file?.name || "")
            ...
            .trim();
        const defaultName =
            String(json?.name || "").trim() || fileBaseName || "导入预设";
        ...
        <div class="header">
            <div class="header-info">
                ...
                <div class="header-desc">
                    参照 SillyTavern：选择/编辑提示词与生成参数，影响 prompt
                    构建
                </div>
            </div>
            <div class="header-actions">
                <button class="btn-action" onclick={handleImport}>导入</button>
                <button class="btn-action" onclick={handleExportCurrent}
                    >导出</button
                >
                <button class="btn-action" onclick={handleExportAll}
                    >导出全部</button
                >
        ...
        <div class="content">
            ...
            <div class="selector-row">
                ...
                <div class="action-btns">
                    <button class="btn-secondary" onclick={onNew}
                        >＋ 新建</button
                    >
                    <button class="btn-secondary" onclick={onRename}
                        >✎ 重命名</button
                    >
            ...
            <div class="editor-area">
                {#if activeType === "sysprompt"}
                    <SyspromptEditor
                        ...
                        bind:this={syspromptEditor}
                    />
                {:else if activeType === "chatprompts"}
                    <ChatPromptsEditor
                        preset={currentPreset}
                        bind:this={chatPromptsEditor}
                    />
                {:else if activeType === "context"}
                    <ContextEditor
                        preset={currentPreset}
                        bind:this={contextEditor}
                    />
                {:else if activeType === "instruct"}
                    <InstructEditor
                        preset={currentPreset}
                        bind:this={instructEditor}
                    />
                ...
                {:else if activeType === "openai"}
                    <OpenAIParamsEditor
                        preset={currentPreset}
                        bind:this={openaiParamsEditor}
                    />
                {:else if activeType === "custom"}
                    <OpenAIBlocksEditor
                        preset={currentPreset}
                        bind:this={openaiBlocksEditor}
                    />
                {/if}
            </div>
            ...
            {#if statusVisible}
                <div
                    class="status"
                    style="background: {getStatusColors(statusType)
                        .bg}; color: {getStatusColors(statusType).fg};"
                >
                    {statusMessage}
                </div>
            {/if}
...
<style>
    ...
    .panel {
        ...
        height: calc(
            100vh - 20px - env(safe-area-inset-top, 0px) -
                env(safe-area-inset-bottom, 0px)
        );
        height: calc(
            100dvh - 20px - env(safe-area-inset-top, 0px) -
                env(safe-area-inset-bottom, 0px)
        );
