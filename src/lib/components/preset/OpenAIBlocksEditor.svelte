<script>
  ...
  function pickPromptOrderBlock(p) {
    ...
    const byId = (id) =>
      arr.find((b) => b && typeof b === 'object' && String(b.character_id) === String(id));
  ...
  function toggleEnabled(identifier, enabled) {
    blocks = blocks.map((b) => (b.identifier === identifier ? { ...b, enabled } : b));
  }
  ...
  function handleDrop(e, targetIdentifier) {
    ...
    const toIndex = blocks.findIndex((b) => b.identifier === targetIdentifier);
...
<div class="editor-section">
  <div class="section-header">
    ...
    <div class="section-desc">与 ST 类似：区块默认折叠，点击展开；可拖拽排序并可新增自定义区块</div>
  </div>
  ...
  <div class="blocks-header">
    ...
    <button type="button" class="add-btn" onclick={addBlock}> ＋ 新增区块 </button>
  </div>

  <div class="blocks-list">
    {#each blocks as block (block.identifier)}
      {@const { isMarker, canEdit, title, roleName, sysPrompt, content } = getBlockData(
        block.identifier
      )}
      <div
        class="block-card"
      ...
      >
        <div class="block-header" onclick={() => toggleCollapsed(block.identifier)}>
          <div class="block-left">
            <span class="collapse-toggle">{block.collapsed ? '▸' : '▾'}</span>
            <span class="drag-handle">☰</span>
            <div class="block-info">
              <div class="block-title">{title}</div>
              <div class="block-subtitle">
                {isMarker ? 'marker（自动填充）' : `role: ${roleName}`}
              </div>
            </div>
          </div>
          <div class="block-right">
            <label class="enable-label" onclick={(e) => e.stopPropagation()}>
              <input
                type="checkbox"
        ...
        {#if !block.collapsed}
          {#if canEdit}
            <div class="block-body">
              <div class="meta-row">
                <div class="meta-cell">
                  <input
                    ...
                    class="field-input"
                    placeholder="区块名称"
                    value={promptById.get(block.identifier)?.name || title}
                    oninput={(e) => updateBlockName(block.identifier, e.target.value)}
                  />
                </div>
                <div class="meta-cell-right">
                  <select
                    class="field-select"
                    value={roleName}
                    onchange={(e) => updateBlockRole(block.identifier, e.target.value)}
                  >
                  ...
                  <label class="sys-prompt-label">
                    <input
                      type="checkbox"
                      checked={sysPrompt}
                      onchange={(e) => updateBlockSystemPrompt(block.identifier, e.target.checked)}
                    />
                    system_prompt
                  </label>
                </div>
              ...
              <div class="content-group">
                <label class="field-label">{block.identifier}</label>
                <textarea
                  class="field-textarea"
                  value={content}
                  oninput={(e) => updateBlockContent(block.identifier, e.target.value)}
                  spellcheck="false"
                ></textarea>
              </div>
            </div>
          {:else}
            <div class="block-body marker-hint">
              该区块为 marker，将在构建 prompt 时自动填充内容（不在此处编辑）。
            </div>
          {/if}
        {/if}
      </div>
