<script>
  ...
  let reasoningMaxAdditions = $state(
    Number.isFinite(Number(settings.reasoningMaxAdditions)) ? settings.reasoningMaxAdditions : 1
  );
...
<div class="editor-section">
  ...
  <div class="flags-row">
    <label class="flag-checkbox">
      <input
        ...
        onchange={() => updateSetting('reasoningAutoParse', reasoningAutoParse)}
      />
      自动解析推理
    </label>
    <label class="flag-checkbox">
      <input
        ...
        onchange={() => updateSetting('reasoningAutoExpand', reasoningAutoExpand)}
      />
      自动展开
    </label>
    <label class="flag-checkbox">
      <input
        ...
        onchange={() => updateSetting('reasoningShowHidden', reasoningShowHidden)}
      />
      显示隐藏推理
    </label>
    <label class="flag-checkbox">
      <input
        ...
        onchange={() => updateSetting('reasoningAddToPrompts', reasoningAddToPrompts)}
      />
      写回提示词
    </label>
  </div>
  ...
  <div class="field-group">
    ...
    <textarea class="field-textarea small" bind:value={prefix} spellcheck="false"></textarea>
  </div>

  <div class="field-group">
    <label class="field-label">推理后缀（suffix）</label>
    <textarea class="field-textarea small" bind:value={suffix} spellcheck="false"></textarea>
  </div>

  <div class="field-group">
    <label class="field-label">推理分隔（separator）</label>
    <textarea class="field-textarea small" bind:value={separator} spellcheck="false"></textarea>
  </div>
</div>
