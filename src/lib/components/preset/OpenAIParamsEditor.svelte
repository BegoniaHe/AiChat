...
<div class="editor-section">
  <div class="section-header">
    ...
    <div class="section-desc">
      参照 ST：编辑常用生成参数；提示词区块请到「自定义」tab 管理（不限制特定
      LLM，可自行绑定连接配置）
    </div>
  </div>
  ...
  <div class="ctx-block">
    ...
    <div class="ctx-hint">用于限制可用上下文窗口（后续可用于自动裁剪历史）。</div>
  </div>
  ...
  <div class="input-row">
    <div class="input-cell">
      ...
      <input type="number" class="field-input" step="0.01" bind:value={temperature} />
    </div>
    <div class="input-cell">
      <label class="field-label">top_p</label>
      <input type="number" class="field-input" step="0.01" bind:value={topP} />
    </div>
    <div class="input-cell">
      <label class="field-label">top_k</label>
      <input type="number" class="field-input" step="1" bind:value={topK} />
    </div>
  </div>
  ...
  <div class="input-row">
    <div class="input-cell">
      <label class="field-label">最大输出 token（max_output_tokens）</label>
      <input type="number" class="field-input" step="1" bind:value={maxTokens} />
    </div>
    <div class="input-cell">
      <label class="field-label">presence_penalty</label>
      <input type="number" class="field-input" step="0.01" bind:value={presencePenalty} />
    </div>
    <div class="input-cell">
      <label class="field-label">frequency_penalty</label>
      <input type="number" class="field-input" step="0.01" bind:value={frequencyPenalty} />
    </div>
  </div>
</div>
