<script>
   ...
   * @component
   */
  import { EXT_PROMPT_ROLES, EXT_PROMPT_TYPES, getInt } from './preset-types.js';
  ...
  let dialoguePosition = $state(preset.dialogue_position ?? EXT_PROMPT_TYPES.SYSTEM_DEPTH_1);
  ...
  let momentCreatePosition = $state(preset.moment_create_position ?? EXT_PROMPT_TYPES.IN_PROMPT);
  let momentCreateDepth = $state(preset.moment_create_depth ?? 1);
  let momentCreateRole = $state(preset.moment_create_role ?? EXT_PROMPT_ROLES.SYSTEM);
  ...
  let momentCommentPosition = $state(preset.moment_comment_position ?? EXT_PROMPT_TYPES.IN_PROMPT);
  let momentCommentDepth = $state(preset.moment_comment_depth ?? 0);
  let momentCommentRole = $state(preset.moment_comment_role ?? EXT_PROMPT_ROLES.SYSTEM);
  let momentCommentRules = $state(preset.moment_comment_rules || '');

  // Group chat
  let groupEnabled = $state(Boolean(preset.group_enabled));
  let groupPosition = $state(preset.group_position ?? EXT_PROMPT_TYPES.SYSTEM_DEPTH_1);
  ...
  let summaryPosition = $state(preset.summary_position ?? EXT_PROMPT_TYPES.SYSTEM_DEPTH_1);
  ...
  $effect(() => {
    ...
    dialoguePosition = preset.dialogue_position ?? EXT_PROMPT_TYPES.SYSTEM_DEPTH_1;
    ...
    momentCreatePosition = preset.moment_create_position ?? EXT_PROMPT_TYPES.IN_PROMPT;
    ...
    momentCommentPosition = preset.moment_comment_position ?? EXT_PROMPT_TYPES.IN_PROMPT;
    momentCommentDepth = preset.moment_comment_depth ?? 0;
    momentCommentRole = preset.moment_comment_role ?? EXT_PROMPT_ROLES.SYSTEM;
    momentCommentRules = preset.moment_comment_rules || '';

    groupEnabled = Boolean(preset.group_enabled);
    groupPosition = preset.group_position ?? EXT_PROMPT_TYPES.SYSTEM_DEPTH_1;
    ...
    summaryPosition = preset.summary_position ?? EXT_PROMPT_TYPES.SYSTEM_DEPTH_1;
  ...
  export function collectData() {
    return {
      ...
      dialogue_position: getInt(dialoguePosition, EXT_PROMPT_TYPES.SYSTEM_DEPTH_1),
      ...
      moment_create_position: getInt(momentCreatePosition, EXT_PROMPT_TYPES.IN_PROMPT),
      moment_create_depth: getInt(momentCreateDepth, 1),
      moment_create_role: getInt(momentCreateRole, EXT_PROMPT_ROLES.SYSTEM),
      moment_create_rules: momentCreateRules,

      moment_comment_enabled: momentCommentEnabled,
      moment_comment_position: getInt(momentCommentPosition, EXT_PROMPT_TYPES.IN_PROMPT),
      moment_comment_depth: getInt(momentCommentDepth, 0),
      moment_comment_role: getInt(momentCommentRole, EXT_PROMPT_ROLES.SYSTEM),
      moment_comment_rules: momentCommentRules,

      group_enabled: groupEnabled,
      group_position: getInt(groupPosition, EXT_PROMPT_TYPES.SYSTEM_DEPTH_1),
      group_depth: getInt(groupDepth, 1),
      group_role: getInt(groupRole, EXT_PROMPT_ROLES.SYSTEM),
      group_rules: groupRules,

      summary_enabled: summaryEnabled,
      summary_position: getInt(summaryPosition, EXT_PROMPT_TYPES.SYSTEM_DEPTH_1),
  ...
  <div class="prompt-list">
    ...
    <div class="prompt-card" class:disabled={!dialogueEnabled}>
      ...
      <div class="card-header" onclick={() => toggleCollapse('dialogue')}>
        <div class="card-left">
          <span class="collapse-toggle">{collapsed.dialogue ? '▸' : '▾'}</span>
          <div class="card-info">
        ...
        <label class="enable-checkbox" onclick={(e) => e.stopPropagation()}>
        ...
        <div class="card-body">
          <div class="input-row">
            <div class="input-cell">
              ...
              <select class="field-select" bind:value={dialoguePosition} disabled>
            ...
            <div class="input-cell">
              ...
              <input type="number" class="field-input" bind:value={dialogueDepth} disabled />
            </div>
            <div class="input-cell">
              <label class="field-label">角色（固定）</label>
              <select class="field-select" bind:value={dialogueRole} disabled>
    ...
    <div class="prompt-card" class:disabled={!momentCreateEnabled}>
      <!-- svelte-ignore a11y_click_events_have_key_events -->
      <!-- svelte-ignore a11y_no_static_element_interactions -->
      <div class="card-header" onclick={() => toggleCollapse('momentCreate')}>
        <div class="card-left">
          <span class="collapse-toggle">{collapsed.momentCreate ? '▸' : '▾'}</span>
          <div class="card-info">
            <div class="card-title">动态发布决策提示词</div>
            <div class="card-subtitle">
              让模型决定是否要输出 moment_start/moment_end（仅用于私聊/群聊场景）
            </div>
          </div>
        </div>
        <label class="enable-checkbox" onclick={(e) => e.stopPropagation()}>
      ...
      {#if !collapsed.momentCreate}
        <div class="card-body">
          <div class="input-row">
            <div class="input-cell">
              <label class="field-label">注入位置</label>
              <select class="field-select" bind:value={momentCreatePosition}>
            ...
            <div class="input-cell">
              ...
              <input type="number" class="field-input" bind:value={momentCreateDepth} min="0" />
            </div>
            <div class="input-cell">
              <label class="field-label">角色（IN_CHAT）</label>
              <select class="field-select" bind:value={momentCreateRole}>
                <option value={EXT_PROMPT_ROLES.SYSTEM}>SYSTEM</option>
                <option value={EXT_PROMPT_ROLES.USER}>USER</option>
                <option value={EXT_PROMPT_ROLES.ASSISTANT}>ASSISTANT</option>
              </select>
    ...
    <div class="prompt-card" class:disabled={!momentCommentEnabled}>
      <!-- svelte-ignore a11y_click_events_have_key_events -->
      <!-- svelte-ignore a11y_no_static_element_interactions -->
      <div class="card-header" onclick={() => toggleCollapse('momentComment')}>
        <div class="card-left">
          <span class="collapse-toggle">{collapsed.momentComment ? '▸' : '▾'}</span>
          <div class="card-info">
            <div class="card-title">动态评论回复提示词</div>
            <div class="card-subtitle">
              仅用于"动态评论"场景：输出 moment_reply_start/moment_reply_end（不输出私聊/群聊）
            </div>
          </div>
        </div>
        <label class="enable-checkbox" onclick={(e) => e.stopPropagation()}>
          <input type="checkbox" bind:checked={momentCommentEnabled} />
          启用
        </label>
      </div>
      {#if !collapsed.momentComment}
        <div class="card-body">
          <div class="input-row">
            <div class="input-cell">
              <label class="field-label">注入位置</label>
              <select class="field-select" bind:value={momentCommentPosition}>
                {#each POSITION_OPTIONS_FULL as opt}
                  <option value={opt.v}>{opt.t}</option>
                {/each}
              </select>
            ...
            <div class="input-cell">
              <label class="field-label">深度（IN_CHAT）</label>
              <input type="number" class="field-input" bind:value={momentCommentDepth} min="0" />
            </div>
            <div class="input-cell">
              <label class="field-label">角色（IN_CHAT）</label>
              <select class="field-select" bind:value={momentCommentRole}>
                <option value={EXT_PROMPT_ROLES.SYSTEM}>SYSTEM</option>
                <option value={EXT_PROMPT_ROLES.USER}>USER</option>
                <option value={EXT_PROMPT_ROLES.ASSISTANT}>ASSISTANT</option>
              </select>
    ...
    <div class="prompt-card" class:disabled={!groupEnabled}>
      ...
      <div class="card-header" onclick={() => toggleCollapse('group')}>
        <div class="card-left">
          <span class="collapse-toggle">{collapsed.group ? '▸' : '▾'}</span>
          <div class="card-info">
      ...
      {#if !collapsed.group}
        <div class="card-body">
          <div class="input-row">
            <div class="input-cell">
              <label class="field-label">注入位置</label>
              <select class="field-select" bind:value={groupPosition} disabled>
                {#each POSITION_OPTIONS_FIXED as opt}
                  <option value={opt.v}>{opt.t}</option>
                {/each}
              </select>
            ...
            <div class="input-cell">
              <label class="field-label">深度（固定）</label>
              <input type="number" class="field-input" bind:value={groupDepth} disabled />
            </div>
            <div class="input-cell">
              <label class="field-label">角色（固定）</label>
              <select class="field-select" bind:value={groupRole} disabled>
                <option value={EXT_PROMPT_ROLES.SYSTEM}>SYSTEM</option>
                <option value={EXT_PROMPT_ROLES.USER}>USER</option>
                <option value={EXT_PROMPT_ROLES.ASSISTANT}>ASSISTANT</option>
              </select>
    ...
    <div class="prompt-card" class:disabled={!summaryEnabled}>
      ...
      <div class="card-header" onclick={() => toggleCollapse('summary')}>
        <div class="card-left">
          <span class="collapse-toggle">{collapsed.summary ? '▸' : '▾'}</span>
          <div class="card-info">
      ...
      {#if !collapsed.summary}
        <div class="card-body">
          <div class="input-row">
            <div class="input-cell">
              <label class="field-label">注入位置</label>
              <select class="field-select" bind:value={summaryPosition} disabled>
                {#each POSITION_OPTIONS_FIXED as opt}
                  <option value={opt.v}>{opt.t}</option>
                {/each}
              </select>
