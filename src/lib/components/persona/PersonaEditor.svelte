<script>
  ...
  import { normalizeHexColor, POSITION_OPTIONS, ROLE_OPTIONS } from './persona-types.js';
  ...
  /** @type {Props} */
  const { persona = null, canDelete = false, onSave, onDelete, onBack } = $props();
  ...
  function handleAvatarClick() {
    ...
    if (useFile) {
      ...
      const url = prompt('请输入头像地址', avatar || './assets/external/feather-default.png');
      if (url !== null) {
        avatar = url.trim();
      }
    }
...
<div class="persona-editor">
  ...
  <div class="editor-body">
    ...
    <div class="avatar-section">
      ...
      <button class="change-avatar-btn" onclick={handleAvatarClick}>更换头像</button>
      <input
        bind:this={fileInput}
        type="file"
        accept="image/*"
    ...
    <div class="field">
      ...
      <input type="text" id="edit-name" bind:value={name} placeholder="输入角色名称" />
    </div>
    ...
    <div class="field">
      ...
      <div class="color-row">
        ...
        />
        <input type="color" id="edit-bubble-color" bind:value={colorInput} />
      </div>
    ...
    <div class="field">
      <label for="edit-desc">
        详细描述 ({'{{persona}}'})
        <span class="label-hint">注入到 System Prompt 或 Character Card 中</span>
      </label>
      <textarea
        id="edit-desc"
      ...
      {#if showDepthSettings}
        <div class="depth-row">
          <div class="field">
            ...
            <input type="number" id="edit-depth" bind:value={depth} min="0" step="1" />
          </div>
          <div class="field">
