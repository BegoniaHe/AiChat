<script>
   ...
   */
  import { contactsStore, convertSTWorld, getRegexStore, getWorldInfoStore } from '$stores';
  ...
  // Computed
  const displayNames = $derived(worldNames.filter((n) => n !== '__builtin_phone_format__'));
  ...
  const activatedWorldId = $derived(currentScope === 'global' ? globalWorldId : currentWorldId);
  ...
  async function refreshList() {
    ...
    // Get session info
    sessionId = getSessionId?.() || window.appBridge?.activeSessionId || 'default';
    const contact = contactsStore.getContact(sessionId);
    groupContact = contact;
    isGroupSession =
      currentScope === 'session' &&
      (Boolean(contact?.isGroup) || String(sessionId).startsWith('group:'));
    ...
    globalWorldId = rawGlobal === '__builtin_phone_format__' ? '' : rawGlobal;
    currentWorldId = rawCurrent === '__builtin_phone_format__' ? '' : rawCurrent;
  }
  ...
  function promptSelectWorld(memberId, memberName) {
    const options = displayNames.slice().sort((a, b) => String(a).localeCompare(String(b)));
  ...
  async function exportWorld(name) {
    ...
    try {
      ...
      const bound = sets
        .filter((s) => s?.bind?.type === 'world' && s.bind.worldId === name)
        .map((s) => ({
          name: s.name,
  ...
  async function importBoundRegexSets(boundSets, worldName) {
    ...
    const ruleSig = (r) => {
      ...
      const minD =
        r?.minDepth === null || r?.minDepth === undefined || r?.minDepth === ''
          ? ''
          : String(r?.minDepth);
      const maxD =
  ...
  <div class="world-panel-content">
    ...
    <div class="panel-columns">
      ...
      <div class="list-column">
        ...
        {#if isGroupSession && groupMembers.length > 0}
          <div class="group-members-section">
            <div class="group-title">群聊世界书（按成员绑定，自动合并 A+B+...）</div>
            ...
            <div class="members-list">
              {#each groupMembers as memberId}
                ...
                <div class="member-row">
                  <img src={info.avatar} alt="" class="member-avatar" />
                  <div class="member-info">
                    <div class="member-name">
                      {info.name}
                    </div>
                    <div class="member-bound" class:unbound={!bound}>
                  ...
                  <div class="member-actions">
                    <button
                      class="btn btn-sm"
                      onclick={() => promptSelectWorld(memberId, info.name)}
                    >
                      {bound ? '更换' : '绑定'}
                    </button>
                    <button
        ...
        <ul class="world-list">
          ...
          {:else}
            {#each displayNames as name}
              ...
              <li class="world-item" class:active={isActive} ondblclick={() => openEditor(name)}>
                <span class="world-name" title="双击编辑世界书">{name}</span>
                <div class="world-actions">
                  {#if isGroupSession && currentScope === 'session'}
                    <button class="btn btn-sm" disabled>（群聊）</button>
                  {:else}
                    <button
                      class="btn btn-sm"
                  ...
                  <button
                    class="btn btn-sm btn-danger"
                    disabled={!activatedWorldId || (isGroupSession && currentScope === 'session')}
                    onclick={deactivateWorld}
                  >
                    停用
                  </button>
                  <button class="btn btn-sm" onclick={() => exportWorld(name)}>导出</button>
                  <button class="btn btn-sm btn-danger-outline" onclick={() => deleteWorld(name)}
                    >删除</button
                  >
                </div>
              </li>
        ...
        <div class="list-actions">
          <button class="btn btn-primary" onclick={createNewWorld}>新增</button>
          <button class="btn" onclick={exportCurrentWorld}>导出当前</button>
        </div>
      </div>
      ...
      <div class="import-column">
        ...
        <div class="import-hint">名称将取自 JSON 的 name 或文件名（无需手动填写）</div>
