...
<div class="world-entry-form">
  ...
  <div class="form-row">
    <div class="form-col">
      ...
      <select id="we-position" bind:value={entry.position} onchange={updateEntry}>
  ...
  <div class="form-row">
    <div class="form-col">
      ...
      <input
        ...
        oninput={() => handleNumber('probability', entry.probability, 0, 100)}
        min="0"
        max="100"
      />
    </div>
    <div class="form-col">
      ...
      <div class="checkbox-group">
        <label class="checkbox-label">
          <input type="checkbox" bind:checked={entry.useProbability} onchange={updateEntry} />
          启用概率
        </label>
      </div>
    </div>
    {#if Number(entry.position) === 4}
      <div class="form-col">
        ...
        <select id="we-role" bind:value={entry.role} onchange={updateEntry}>
  ...
  <div class="form-section">
    ...
    <div class="checkbox-group">
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.disable} onchange={updateEntry} />
        禁用（红灯）
      </label>
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.constant} onchange={updateEntry} />
        常驻（蓝灯）
      </label>
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.selective} onchange={updateEntry} />
        选择性触发（绿灯）
      </label>
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.ignoreBudget} onchange={updateEntry} />
        忽略预算
      </label>
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.excludeRecursion} onchange={updateEntry} />
        不参与递归
      </label>
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.preventRecursion} onchange={updateEntry} />
        阻止递归
      </label>
    </div>

    <label style="margin-top: 8px;">选择性逻辑（Selective Logic）</label>
    <select id="we-selectiveLogic" bind:value={entry.selectiveLogic} onchange={updateEntry}>
  ...
  <div class="form-section">
    ...
    <div class="checkbox-group">
      ...
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.matchScenario} onchange={updateEntry} />
        场景
      </label>
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.matchCreatorNotes} onchange={updateEntry} />
        作者注释
      </label>
    </div>
  </div>
  ...
  <div class="form-section">
    <div class="form-row">
      ...
      <div class="form-col">
        ...
        <input
          ...
          oninput={() => handleNumber('groupWeight', entry.groupWeight, 0, 9999)}
          min="0"
          max="9999"
        />
      </div>
    ...
    <div class="checkbox-group" style="margin-top: 6px;">
      <label class="checkbox-label">
        <input type="checkbox" bind:checked={entry.groupOverride} onchange={updateEntry} />
        允许覆盖同组
      </label>
      <label class="checkbox-label">
        <input
          ...
          onchange={(e) => handleOverrideCheck('caseSensitive', e.target.checked)}
        />
        区分大小写（覆盖）
      </label>
      <label class="checkbox-label">
        <input
          ...
          onchange={(e) => handleOverrideCheck('matchWholeWords', e.target.checked)}
        />
        全词匹配（覆盖）
      </label>
      <label class="checkbox-label">
        <input
          ...
          onchange={(e) => handleOverrideCheck('useGroupScoring', e.target.checked)}
        />
        组打分（覆盖）
      </label>
    </div>
