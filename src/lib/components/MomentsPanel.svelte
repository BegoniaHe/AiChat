<script>
    ...
    function getAvatarByName(name) {
        ...
        const match = contacts.find(
            (x) => norm(x?.name) === key || norm(x?.id) === key,
        );
    ...
    function buildThreadedComments(comments = []) {
        ...
        for (const c of list) {
            ...
            if (replyTo && byId.has(replyTo)) {
                if (!repliesByParent.has(replyTo))
                    repliesByParent.set(replyTo, []);
...
<div class="moments-panel">
    ...
    {:else}
            ...
            {#each visibleMoments as moment (moment.id)}
                ...
                {@const visibleComments = expanded
                    ...
                    : hiddenCount > 0
                      ...
                      : comments}
                {@const { roots: commentRoots, repliesByParent } =
                    buildThreadedComments(visibleComments)}
                ...
                <div class="moment-card" data-moment-id={moment.id}>
                    ...
                    <div class="moment-footer">
                        <span class="moment-likes"
                            >👍 {moment.likes || 0}人已赞</span
                        >
                        <button
                            class="moment-action"
                            onclick={() => toggleComposer(moment.id)}
                        >
                            评论
                        </button>
                    </div>
                                ...
                                {#each replies as reply}
                                    ...
                                    >
                                        <span class="comment-user">
                                            ...
                                            回复
                                            <span class="comment-replyto"
                                                >{toName}</span
                                            >：
                                        </span>
                                        <span class="comment-text">
                                            {@html renderTextWithStickers(
                                                rcontent,
                                            )}
                                        </span>
                                    </div>
                            ...
                            {#if replyTarget}
                                <div class="moment-replying">
                                    <div class="reply-info">
                                        回复 <b>{replyTarget.author || ""}</b
                                        >：{String(
                                            replyTarget.content || "",
                                        ).slice(0, 120)}
                                    </div>
                                    <button
                                        class="moment-reply-cancel"
                                        onclick={() => cancelReply(moment.id)}
                                    >
                                        ×
                                    </button>
                                </div>
        ...
        {#if hasMore}
            <button class="moments-load-more" onclick={loadMore}>
                加载更多
            </button>
        {/if}
    {/if}
