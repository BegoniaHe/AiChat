...
function getValidParentOptions(excludeId) {
    ...
    function collectDescendants(id) {
        descendants.add(id);
        groups.filter(g => g.parentId === id).forEach(child => {
            ...
            {:else}
                <div class="group-list">
                    {#each flatGroups as { group, depth } (group.id)}
                        ...
                        {@const parentName = group.parentId ? tree.byId.get(group.parentId)?.name : ''}
                            ...
                            {:else}
                                ...
                                <div class="group-info">
                                    ...
                                    <div class="group-meta">
                                        {count} 个联系人
                                        {#if parentName}
                                            <span class="parent-info">· 上级：{parentName}</span>
                                        {/if}
                                    </div>
                                </div>
                                <div class="group-actions">
                                    <select 
                                        ...
                                        onchange={(e) => changeParent(group.id, e.target.value)}
                                    >
                                    ...
                                    <button class="edit-btn" onclick={() => startEdit(group)}>
                                        编辑
                                    </button>
