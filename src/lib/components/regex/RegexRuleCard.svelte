...
<div class="rule-card" class:collapsed class:disabled={rule.disabled} data-rule-id={rule.id}>
  ...
  <div class="rule-header" onclick={toggleCollapsed}>
    ...
    <div class="rule-right" onclick={(e) => e.stopPropagation()}>
      <label class="enabled-label">
        <input type="checkbox" checked={!rule.disabled} onchange={handleEnabledChange} />
        启用
      </label>
    ...
    <div class="rule-body">
      ...
      <div class="form-row">
        <div class="form-col" style="flex: 1; min-width: 220px;">
          <label>脚本名称</label>
          <input type="text" bind:value={rule.scriptName} oninput={() => onUpdate?.()} />
        </div>
      ...
      <div class="form-row">
        ...
        <div class="form-section" style="flex: 1; min-width: 260px;">
            ...
            {#each PLACEMENT_OPTIONS as opt}
              <label class="checkbox-label">
                <input
                  ...
                  onchange={(e) => handlePlacementChange(opt.value, e.target.checked)}
                />
                {opt.label}
              </label>
            {/each}
          ...
          <div class="depth-row">
            ...
            <input
              ...
              placeholder="Min"
              value={rule.minDepth ?? ''}
              oninput={(e) => handleDepthChange('minDepth', e.target.value)}
            />
            <input
              type="number"
              min="0"
              ...
              placeholder="Max"
              value={rule.maxDepth ?? ''}
              oninput={(e) => handleDepthChange('maxDepth', e.target.value)}
            />
        ...
        <div class="form-section" style="flex: 1; min-width: 260px;">
          ...
          <div class="options-list">
            <label class="checkbox-label">
              <input type="checkbox" bind:checked={rule.disabled} onchange={() => onUpdate?.()} />
              停用（Disabled）
            </label>
            <label class="checkbox-label">
              <input type="checkbox" bind:checked={rule.runOnEdit} onchange={() => onUpdate?.()} />
              编辑消息时执行（Run On Edit）
            </label>

            <div class="select-row">
              <span>Find Regex 宏</span>
              <select bind:value={rule.substituteRegex} onchange={() => onUpdate?.()}>
            ...
            <div class="ephemerality-section">
              <label class="section-subtitle">暂时性（Ephemerality）</label>
              <label class="checkbox-label">
                <input
                  type="checkbox"
              ...
              <div class="hint">两者都不勾选：将直接修改聊天存档内容（不可逆）。</div>
            </div>
          </div>
        </div>
      </div>
