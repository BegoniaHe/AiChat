...
export function normalizeScript(r = {}) {
    ...
    return {
        ...
        substituteRegex: (r.substituteRegex === 1 || r.substituteRegex === 2)
            ? Number(r.substituteRegex)
            : 0,
        minDepth: (r.minDepth === '' || r.minDepth === undefined)
            ? null
            : (r.minDepth === null ? null : Number(r.minDepth)),
        maxDepth: (r.maxDepth === '' || r.maxDepth === undefined)
            ? null
            : (r.maxDepth === null ? null : Number(r.maxDepth)),
    };
}
