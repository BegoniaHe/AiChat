<script>
    ...
    async function refreshGlobal() {
        ...
        globalRules = (Array.isArray(g.rules) ? g.rules : []).map((r) =>
            normalizeScript(r),
        );
    }
    ...
    async function refreshLocal() {
        ...
        if (activeLocalSetId) {
            ...
            if (currentLocalSet) {
                ...
                localRules = (
                    Array.isArray(currentLocalSet.rules)
                        ? currentLocalSet.rules
                        : []
                ).map((r) => normalizeScript(r));

                // Load binding
                if (currentLocalSet.bind?.type === "preset") {
                    bindType = "preset";
                    bindPresetType =
                        currentLocalSet.bind.presetType || "openai";
    ...
    async function deleteLocalSet() {
        ...
        if (!confirm(`删除局部正则「${cur?.name || activeLocalSetId}」？`))
            return;
...
<Modal bind:open={visible} title="正规表达式" size="xl" closable>
    <div class="regex-panel">
        ...
        <div class="panel-desc">
            参考
            ST：按规则替换输入/输出文本；支持全局与绑定预设/世界书的局部规则
        </div>
                ...
                <div class="global-content">
                    <div class="toolbar">
                        ...
                        <div class="toolbar-actions">
                            <button class="btn" onclick={addGlobalRule}
                                >＋ 新增规则</button
                            >
                    ...
                    <div class="rules-list">
                        {#each globalRules as rule, i (rule.id)}
                            <RegexRuleCard
                                bind:rule={globalRules[i]}
                                onDelete={() => deleteGlobalRule(i)}
                            />
                        {/each}
                ...
                <div class="local-content">
                    ...
                    <div class="sets-column">
                        ...
                        <div class="sets-toolbar">
                            <button class="btn" onclick={createLocalSet}
                                >＋ 新建</button
                            >
                            <button
                                class="btn btn-danger"
                                onclick={deleteLocalSet}>删除</button
                            >
                        </div>
                        <div class="sets-list">
                        ...
                        {#if currentLocalSet}
                            <div class="editor-header">
                                <div class="editor-info">
                                    ...
                                    <div class="editor-desc">
                                        绑定到预设或世界书后，切换到对应对象时自动生效
                                    </div>
                                </div>
                                <div class="editor-actions">
                                    <button class="btn" onclick={renameLocalSet}
                                        >✎ 重命名</button
                                    >
                                    <button
                                        class="btn btn-primary"
                                        onclick={saveLocal}>保存</button
                                    >
                                </div>
                            </div>
                            ...
                            <div class="editor-options">
                                ...
                                <div class="bind-row">
                                    ...
                                    {#if bindType === "preset"}
                                        <select
                                            bind:value={bindPresetType}
                                            onchange={updatePresetList}
                                        >
                            ...
                            <div class="rules-toolbar">
                                ...
                                <button class="btn" onclick={addLocalRule}
                                    >＋ 新增规则</button
                                >
                            </div>
                            ...
                            <div class="rules-list">
                                {#each localRules as rule, i (rule.id)}
                                    <RegexRuleCard
                                        bind:rule={localRules[i]}
                                        onDelete={() => deleteLocalRule(i)}
                                    />
                                {/each}
