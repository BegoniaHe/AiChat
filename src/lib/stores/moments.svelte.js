/**
 * Moments Store (动态/朋友圈)
 * 迁移自: src/scripts/storage/moments-store.js
 */
...
export class MomentsStore {
        ...
        try {
          ...
          if (isQuotaError(err)) {
            ...
            if (!this._lsQuotaWarned) {
              ...
              logger.warn(
                'moments store: localStorage quota exceeded; will rely on Tauri KV.',
                err
              );
            }
            try {
      ...
      try {
        ...
        if (isQuotaError(err)) {
          ...
          if (!this._lsQuotaWarned) {
            this._lsQuotaWarned = true;
            logger.warn(
              'moments store: localStorage quota exceeded; disabling localStorage writes.',
              err
            );
          }
          try {
  ...
  upsert(moment) {
    ...
    const authorId = hasOwn(moment, 'authorId')
      ? String(moment.authorId || '').trim()
      : existingById?.authorId || existingBySig?.authorId || '';
    const author = hasOwn(moment, 'author')
      ? String(moment.author || '').trim()
      : existingById?.author || existingBySig?.author || '';
    const content = hasOwn(moment, 'content')
      ? String(moment.content || '')
      : existingById?.content || existingBySig?.content || '';
    const time = hasOwn(moment, 'time')
      ? String(moment.time || '')
      : existingById?.time || existingBySig?.time || '';
    const originSessionId = hasOwn(moment, 'originSessionId')
      ? String(moment.originSessionId || '').trim()
      : existingById?.originSessionId || existingBySig?.originSessionId || '';
    const views = hasOwn(moment, 'views')
      ? Number.isFinite(Number(moment.views))
        ? Number(moment.views)
        : 0
      : Number.isFinite(Number(existingById?.views))
        ? Number(existingById.views)
        : Number.isFinite(Number(existingBySig?.views))
          ? Number(existingBySig.views)
          : 0;
    const likes = hasOwn(moment, 'likes')
      ? Number.isFinite(Number(moment.likes))
        ? Number(moment.likes)
        : 0
      : Number.isFinite(Number(existingById?.likes))
        ? Number(existingById.likes)
        : Number.isFinite(Number(existingBySig?.likes))
          ? Number(existingBySig.likes)
          : 0;
    ...
    const comments = hasOwn(moment, 'comments')
      ...
      : normalizeComments(
          Array.isArray(existingById?.comments)
            ? existingById.comments
            : Array.isArray(existingBySig?.comments)
              ? existingBySig.comments
              : []
        );
    ...
    const timestamp = hasOwn(moment, 'timestamp')
      ? Number.isFinite(Number(moment.timestamp))
        ...
        : Date.now()
      : Number.isFinite(Number(existingById?.timestamp))
        ? Number(existingById.timestamp)
        : Number.isFinite(Number(existingBySig?.timestamp))
          ? Number(existingBySig.timestamp)
          : Date.now();

    const signature =
      sig ||
      existingById?.signature ||
      existingBySig?.signature ||
      `${author}\u0000${content}\u0000${time}`;
  ...
  addComments(momentId, comments = []) {
    ...
    (Array.isArray(comments) ? comments : []).forEach((c) => {
      ...
      const cid =
        proposedId && !existing.some((x) => String(x?.id || '').trim() === proposedId)
          ? proposedId
          : genId('comment');
      existing.push({
        id: cid,
...
// 创建单例
let momentsStoreInstance = null;

export function getMomentsStore(options = {}) {
  if (!momentsStoreInstance) {
    momentsStoreInstance = new MomentsStore(options);
  }
  return momentsStoreInstance;
}
