...
export class RegexStore {
      ...
      const replaceWithGroups = replaceString.replace(
        ...
        (_m, num, groupName) => {
          ...
          if (num) {
            ...
            match =
              groups && typeof groups === 'object' && groups[groupName] ? groups[groupName] : '';
          }
          if (!match) return '';
  ...
  apply(
    ...
    ctx = {},
    placement,
    { isMarkdown = false, isPrompt = false, isEdit = false, depth } = {}
  ) {
    const raw = String(text ?? '');
    if (!raw) return raw;
    ...
    for (const s of scripts) {
      ...
      if (typeof depth === 'number' && Number.isFinite(depth)) {
        const minD =
          typeof s.minDepth === 'number' && Number.isFinite(s.minDepth) ? s.minDepth : null;
        const maxD =
          typeof s.maxDepth === 'number' && Number.isFinite(s.maxDepth) ? s.maxDepth : null;
