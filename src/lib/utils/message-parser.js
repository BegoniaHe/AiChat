...
import { isLikelyUrl, resolveMediaAsset } from './media-assets.js';

/** @type {RegExp} */
const TOKEN_PATTERN = /^\[(img|yy|music|zz|bqb)-([\s\S]+)\]$/i;

/**
 * Attach asset metadata if available
 * @param {{ item?: { id?: string, kind?: string, label?: string } } | null | undefined} resolved - Resolved media asset
 * @returns {{ assetId: string, assetKind: string, assetLabel: string } | undefined} - Asset metadata
 */
function attachAssetMeta(resolved) {
...
/**
 * Parse special message tokens from assistant responses
 * @param {string} raw - Raw message content
 * @returns {{ type: string, content: string, meta?: Object }} - Parsed message
 */
export function parseSpecialMessage(raw = '') {
  ...
  switch (type) {
    case 'img': {
      ...
      const isUrl =
        isLikelyUrl(payload) || payload.startsWith('./') || payload.startsWith('/assets');
      if (isUrl) {
